...
/**
 * Test-only `AbstractOtpDriver` shape that records every `dispatch()` call
 * against an externally observable list and tracks the dispatch / persist call
 * order through a shared array reference.
 *
 * Lives in `tests/Fixtures` so the unit-test driver factory can lock a concrete
 * return type without resorting to inline `object{...}` types — which exceed
 * PHPDoc line-length limits and trip Slevomat's FQN rule when expressed via
...
abstract class DispatchTrackingOtpDriver extends AbstractOtpDriver
{
    ...
    /**
     * Bind the order-tracker to an external array reference so test assertions
     * can observe call ordering without touching the driver.
     *
